...
exposedModulesTests = testGroup "exposedBy"
  ...
  , exposureCase "a private sublibrary doesn't expose anything"
      ["pkg/internal"]
      [("pkg/pkg.cabal", unlines (libLines ["src"] [] [] ++ privateSubLibLines ["internal"] ["Secret"]))]
      "pkg/internal/Secret.hs" "Secret" False
  , testCase "a file defaults to exposed" $ do
      fp <- encodeFS "/nowhere/Foo.hs"
      exposedBy fp "Foo" [] @?= True
  ]

-- | Creates @dirs@ under a canonical temp dir, then parses each @(path, text)@
-- as the cabal file at that path.
exposureCase
...
exposureCase name dirs cabals file modName want =
  testCase name $ withCanonicalTempDir $ \tmp -> do
    ...
    comps <- concat <$> traverse
      ...
      cabals
    fp <- encodeFS (tmp </> file)
    let got = exposedBy fp modName comps
...
-- | @header@ then the fields every stanza kind shares, minus the empty ones.
