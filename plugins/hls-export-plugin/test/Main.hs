...
-- | A package whose test-suite is a separate component, so on-demand loading
-- leaves the test-suite's @Spec.hs@ out of the session.
runMulti :: (FilePath -> Session a) -> IO a
runMulti = runProject "testdata-multi"

runMultiOnDemand :: (FilePath -> Session a) -> IO a
runMultiOnDemand = runProjectWith PreferMultiComponentLoading "testdata-multi"
...
refineCase name = refineCaseAt name (rangeAt 0 7)

-- | 'refineCase' with the module header somewhere other than the first line.
refineCaseAt :: TestName -> Range -> FilePath -> FilePath -> [T.Text] -> [T.Text] -> TestTree
refineCaseAt name header consumer target present absent =
    testCase name $ runExposed $ \_dir -> do
        _       <- openDoc consumer "haskell"
        target' <- openDoc target "haskell"
        waitForIndex consumer
        ca <- findAction (== explicitly) target' header
...
main = defaultTestRunner $ testGroup "Export"
    ...
    , testGroup "Export explicitly"
        [ testGroup "refines to externally-referenced names"
            ...
            , refineCaseAt "a CPP module with export list without directives"
                (rangeAt 1 7) "UsesCppExportNoDirective.hs" "CppExportNoDirective.hs"
                ["(alpha)"] ["beta"]
            ]
        , testGroup "offered only where a safe rewrite exists"
            ...
            , -- See Note [Every consumer must be visible].
              testCase "not offered when components load on demand" $
                runMultiOnDemand $ \_dir -> do
                    target <- openDoc ("src" </> "Lib.hs") "haskell"
                    waitForKickDone
                    titleOffered False target (rangeAt 0 7)
            , testCase "offered under whole-project loading, which loads the sibling component" $
                runMulti $ \_dir -> do
                    target <- openDoc ("src" </> "Lib.hs") "haskell"
                    waitForKickDone
                    titleOffered True target (rangeAt 0 7)
            ]
        ]
    ]
